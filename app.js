...
{
    e.preventDefault();/* ensure that the page don't reload when the form submited*/
    addTodo();
})

function addTodo()
{
    const todoText = todoInput.value.trim();/*trim() ensure that we remove white space from the text we enter*/
...
{
    todoListUL.innerHTML = "";/* clear our todo ul list */
...
function createTodoItem(todo,todoIndex)
{
    ...
    {
        allTodos[todoIndex].complited = checkbox.checked;
        saveTodos();
    })
...
function getTodos()
{
    ...
    return (JSON.parse(todos));/*convert the string back into in array*/
}